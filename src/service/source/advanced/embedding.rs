...
impl EmbeddingProvider {
    pub fn from_config(config: &AdvancedSourceConfig) -> Result<Option<Self>> {
        ...
        // A key resolved from the gitignored `minds-secrets.yaml` store (via a
        // `${name}` reference or literal in minds.yaml) takes precedence;
        // otherwise fall back to the configured environment variable.
        let api_key = match config.embedding_api_key_resolved.as_deref().filter(|value| !value.trim().is_empty()) {
            Some(key) => Some(key.to_string()),
            None => match config.embedding_api_key_env.as_deref() {
