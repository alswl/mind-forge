...
fn enable_imports_legacy_registrations_and_activates_lance() {
    ...
    // Two legacy file sources plus the two projects' `project` kind (spec 071 US2).
    assert_eq!(data["registrations_total"], 4, "sync must import both sources and both project kinds: {stdout}");
...
fn sync_persists_shared_content_offline() {
    ...
    let data = report(&stdout);
    // Two file sources + two `project` kinds (one per project, spec 071 US2).
    assert_eq!(data["registrations_total"], 4, "both sources and both project kinds processed: {stdout}");
    assert_eq!(data["registrations_added"], 4, "all four registrations newly synced: {stdout}");
...
fn basic_search_matches_metadata_not_content() {
    ...
    // "beta" matches the beta-notes source and the project:beta kind (spec 071 US2).
    assert_eq!(meta["results"].as_array().map(|r| r.len()), Some(2), "metadata name match: {meta_out}");
    assert!(
...
fn status_reports_lance_backend_and_counts() {
    ...
    // Two file sources + two `project` kinds (spec 071 US2).
    assert_eq!(data["registrations_count"], 4, "sources and project kinds in the corpus: {stdout}");
    assert_eq!(data["documents_count"], 4, "one shared document per registration after sync: {stdout}");
