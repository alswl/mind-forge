...
fn sync_lance_catalog(
    ...
    // Restore committed Claude enrichments (`.mind-forge/enrichments/`) into the
    // rebuilt cache. Idempotent, so an unchanged sync stays a no-op.
    if !dry_run && let Err(e) = super::enrichment::restore_into_store(store, repo_root) {
        warnings.push(format!("enrichment restore from .mind-forge/enrichments failed: {e}"));
    }
...
fn sync_one_source(
    ...
    // Determine this sync's content revision: the next revision when the
    // registration already has prior content (a change), else the first. The
    // unchanged case was already skipped above.
    let revision = match store {
        Some(s) => s.current_relation(registration_key)?.map(|(rev, _)| rev + 1).unwrap_or(1),
        None => 1,
    };
...
pub(crate) fn open_active_store(repo_root: &Path) -> Result<LanceStore> {
    ...
    Ok(LanceStore::open(&advanced_dir.join(relative))?
        .with_dimension(super::config::embedding_dimension_for(repo_root)))
}
...
mod tests {
    ...
    fn recover_nonexistent_snapshot_is_error() {
        let dir = tempfile::tempdir().unwrap();
        let advanced = crate::service::source::advanced::advanced_store_dir(dir.path());
    ...
    fn lance_sync_persists_document_relation_and_chunks() {
        ...
        // Enrichment is persisted to the committed authority file, and restore
        // repopulates the cache after loss (idempotent when already present).
        let doc_key = jobs[0].document_key.clone();
        assert!(dir.path().join(".mind-forge/enrichments").join(format!("{doc_key}.json")).exists());
        store.delete_rows("enrichments", "content_revision >= 0").unwrap();
        assert_eq!(store.count_rows("enrichments").unwrap(), 0);
        let restored = crate::service::source::advanced::enrichment::restore_into_store(&store, dir.path()).unwrap();
        assert_eq!(restored, 1);
        assert_eq!(store.count_rows("enrichments").unwrap(), 1);
        assert_eq!(
            crate::service::source::advanced::enrichment::restore_into_store(&store, dir.path()).unwrap(),
            0,
            "restore is a no-op when the enrichment is already present"
        );
    ...
    #[test]
    fn sync_versions_changed_content_and_search_defaults_to_current() {
        use crate::model::source_search::SearchMode;
        let dir = tempfile::tempdir().unwrap();
        let project_dir = dir.path().join("projects/alpha");
        std::fs::create_dir_all(project_dir.join("sources")).unwrap();
        std::fs::write(project_dir.join("sources/notes.md"), "photosynthesis chloroplast\n").unwrap();
        std::fs::write(
            project_dir.join("mind-index.yaml"),
            "project: alpha\nsources:\n  - name: notes\n    kind: file\n    path: sources/notes.md\n",
        )
        ...
        assert_eq!(sync_repository(dir.path(), &lance, None, None, false, false).unwrap().registrations_added, 1);
        // Change the content → a new revision, with the old one retained.
        std::fs::write(project_dir.join("sources/notes.md"), "transformer attention\n").unwrap();
        assert_eq!(sync_repository(dir.path(), &lance, None, None, false, false).unwrap().registrations_added, 1);

        let search = |query: &str, revision: Option<&str>| {
            crate::service::source::advanced::retrieval::search_repository(
                dir.path(),
                query,
                SearchMode::Advanced,
                None,
        ...
        // Default search returns only the current (revision 2) content.
        assert_eq!(search("transformer", None), 1, "current content is found");
        assert_eq!(search("photosynthesis", None), 0, "old revision is hidden by default");
        // `--revision 1` surfaces the retained historical content.
        assert_eq!(search("photosynthesis", Some("1")), 1, "revision 1 is retained and queryable");
        assert_eq!(search("transformer", Some("1")), 0, "current content is not in revision 1");
    }
