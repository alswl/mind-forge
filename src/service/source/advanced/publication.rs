...
// `.mind-forge/` mixes committed config (renders/publisher/enrichments) with the
// gitignored `cache/` subtree, so the ignore policy scopes exactly `cache/`.
const GITIGNORE_CONTENT: &str = "/cache/\n";
...
/// Atomically create `.mind-forge/.gitignore` with the runtime ignore policy.
///
/// `advanced_dir` is `<repo>/.mind-forge/cache/source/advanced`; the ignore
/// policy belongs to the `.mind-forge` root (three levels up), never repo root.
pub fn ensure_gitignore(advanced_dir: &Path) -> Result<()> {
    let mind_forge_dir = advanced_dir
        .parent()
        .and_then(Path::parent)
        .and_then(Path::parent)
        .ok_or_else(|| MfError::advanced_store("invalid advanced dir path".to_string(), None))?;
    fs::create_dir_all(mind_forge_dir)?;
    let gitignore_path = mind_forge_dir.join(GITIGNORE);
