...
pub fn update_registration(
    ...
    let registration = SourceRegistration {
        ...
        // Preserve captured provenance across rename; context regenerated by sync.
        context_json: None,
        imported_by_json: current.imported_by_json.clone(),
        // A rename is a mutation of the same logical record: keep its
        // creation time, refresh its modification time (spec 075 FR-011).
        added_at: current.added_at.clone(),
        updated_at: Some(chrono::Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string()),
        extras_json: current.extras_json.clone(),
    };
    ...
    // A rename replaces the old name with a new one under the same logical
    // record — the old YAML name is explicitly superseded here, so it is a
    // permitted removal rather than unexplained divergence (spec 075 I-1).
    let permitted_removals = if next_name != identity_name { vec![identity_name.to_string()] } else { Vec::new() };
    super::compatibility::export_project_with_removals(
        repo_root,
...
/// Validate that `raw` is a project-relative path that stays within
/// `project_path`, returning it normalized (forward slashes, no leading `./`).
/// Absolute paths or any `..` escape are usage errors (spec 071 contract).
fn validate_project_relative(project_path: &Path, raw: &str) -> Result<String> {
    let candidate = Path::new(raw);
    if candidate.is_absolute() {
        return Err(MfError::usage(format!("--article path '{raw}' must be project-relative, not absolute"), None));
    }
    ...
    for component in candidate.components() {
        match component {
            ...
            _ => {
                return Err(MfError::usage(format!("--article path '{raw}' escapes the project boundary"), None));
            }
        }
    }
    // Guard against symlink/`..` escapes when the target exists.
    let joined = project_path.join(&normalized);
    if joined.exists()
        && let (Ok(real), Ok(base)) = (joined.canonicalize(), project_path.canonicalize())
...
pub fn add_registration(
    ...
    // Capture authoritative source←article import provenance (spec 071). The
    // project is always recorded; the originating article is validated to stay
    // within the project before it is persisted.
    let imported_by_json = {
        let article = match article {
            Some(raw) => Some(validate_project_relative(project_path, raw)?),
            None => None,
        };
        serde_json::to_string(&crate::model::source_advanced::ImportProvenance {
    ...
    let registration = SourceRegistration {
        ...
        context_json: None,
        imported_by_json,
        // Overwriting an existing name is a mutation of the same logical
        // record: keep its creation time. A brand-new registration gets a
        // fresh one. Either way `updated_at` reflects this write (FR-011).
        added_at: existing_by_name
            .and_then(|row| row.added_at.clone())
...
fn project_identity(project_path: &Path) -> Result<String> {
    ...
    Ok(index
        .extra
        .as_ref()
        .and_then(|extra| extra.iter().find(|(k, _)| k == "project").map(|(_, v)| v))
...
pub fn rename_registration(
    ...
    let write_result = (|| -> Result<()> {
        ...
        store.delete_rows("registrations", &format!("registration_key = '{}'", current.registration_key))?;
        store.append_registrations(&[registration])?;
        // The old name is explicitly superseded by this rename, so its YAML
        // entry is a permitted removal, not unexplained divergence (I-1).
        let permitted_removals = if old_name != new_name { vec![old_name.to_string()] } else { Vec::new() };
        super::compatibility::export_project_with_removals(
            repo_root,
            &current.project_identity,
            false,
...
pub fn remove_registration(
    ...
    if !dry_run {
        ...
        // `mf source remove` is the explicit removal command (FR-016): its
        // target name is a permitted removal, not divergence to preserve.
        super::compatibility::export_project_with_removals(
            repo_root,
            &row.project_identity,
            false,
            std::slice::from_ref(&row.source_identity),
        )?;
    }
...
pub fn clean_registrations(repo_root: &Path, project_path: &Path, dry_run: bool) -> Result<SourceIndexReport> {
    ...
    if !dry_run && !stale.is_empty() {
        ...
        // `clean` is the explicit stale-cleanup command: the names it just
        // dropped are permitted removals, not divergence to preserve.
        let permitted_removals: Vec<String> = stale.iter().map(|row| row.source_identity.clone()).collect();
        super::compatibility::export_project_with_removals(
            repo_root,
            &stale[0].project_identity,
            false,
            &permitted_removals,
        )?;
    }
...
/// Reconcile a project's on-disk source files with the Lance primary catalog:
/// files found on disk (or already named in the legacy `sources:` index) but
/// absent from the store are imported; store rows whose file has vanished are
/// reported as missing, never removed (spec 075 US2, I-1/FR-014/FR-015/FR-016).
/// This replaces the old `sync_repository`-backed dispatch, which read only
/// the existing `sources:` list and could never see a genuinely new file
/// (spec 075 D4).
pub fn reconcile_and_adopt(repo_root: &Path, project_path: &Path, dry_run: bool) -> Result<SourceIndexReport> {
    let config = super::config::load_repository_config(repo_root)?;
    if !config.is_lance() {
        return Err(MfError::usage("Lance primary mutation requires an active Lance backend".to_string(), None));
    }
    ...
    let project_identity = project_identity(project_path)?;
    let rows = catalog.registrations(Some(&store))?;
    let project_rows: Vec<CatalogRegistration> =
        rows.into_iter().filter(|row| row.project_path == project_rel).collect();
    let registered_paths: std::collections::BTreeSet<&str> =
        project_rows.iter().map(|row| row.registered_location.as_str()).collect();
    ...
    let disk_files = crate::service::source::index::scan_disk_sources(project_path)?;
    let sources_dir = project_path.join(&config_layout_sources(project_path)?);

    let mut missing: Vec<SourceIndexEntry> = project_rows
        .iter()
        .filter(|row| {
            !row.registered_location.starts_with("http://")
                && !row.registered_location.starts_with("https://")
    ...
    let mut existing_names: std::collections::HashSet<String> =
        project_rows.iter().map(|row| row.source_identity.clone()).collect();
    let mut new_rows: Vec<SourceRegistration> = Vec::new();
    let mut imported: Vec<SourceIndexEntry> = Vec::new();
    let now = chrono::Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string();

    for disk in &disk_files {
        if registered_paths.contains(disk.path.as_str()) {
            continue;
        }
        ...
        if existing_names.contains(&name) {
            let suggestion =
                preserved.is_none().then(|| crate::service::source::add::suggest_unique_name(&full_path, &sources_dir));
            return Err(crate::service::source::add::name_collision_error(&name, suggestion));
        }
        let source_type = crate::service::source::infer_kind_from_path(Path::new(&disk.path)).as_str().to_string();
        let registration_key = identity::registration_key(&project_key, &source_type, &disk.path);
        imported.push(SourceIndexEntry {
            name: name.clone(),
            kind: crate::service::source::infer_kind_from_path(Path::new(&disk.path)),
            path: disk.path.clone(),
        });
        if !dry_run {
            new_rows.push(SourceRegistration {
                registration_key,
                ...
                source_type,
                source_kind: source_kind_name(disk.source_kind.as_ref()),
                registered_location: disk.path.clone(),
                tags_json: "[]".to_string(),
                labels_json: "{}".to_string(),
                annotations_json: "{}".to_string(),
                fact_fingerprint: identity::raw_fingerprint(format!("{name}\n\n{}\n", disk.path).as_bytes()),
                registration_revision: 1,
                state: RegistrationState::Live,
                context_json: None,
    ...
    if dry_run {
        ...
        return Ok(SourceIndexReport { added: imported, removed: missing, kept_count, dry_run: true });
    }
...
fn config_layout_sources(project_path: &Path) -> Result<std::path::PathBuf> {
    let layout = crate::service::config::effective_layout(project_path)?;
    Ok(Path::new(&layout.sources).to_path_buf())
}
