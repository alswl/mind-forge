...
pub fn update_registration(
    ...
    let registration = SourceRegistration {
        ...
        labels_json: current.labels_json.clone(),
        annotations_json: current.annotations_json.clone(),
...
pub fn add_registration(
    ...
    let write_result = (|| -> Result<bool> {
        ...
        store.append_registrations(&[registration])?;
        let projection = super::compatibility::export_project(repo_root, &project_identity, false)?;
        Ok(!projection.drift_details.is_empty())
    })();
    let projection_degraded = match write_result {
