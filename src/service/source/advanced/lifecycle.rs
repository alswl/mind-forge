...
//! records a durable JSON intent under `.mind-forge/cache/source/advanced/transactions/`
...
/// Return the current Lance generation ID for use as a baseline in intents.
/// Returns `None` if Lance is not active or no pointer exists.
pub fn current_baseline_id(repo_root: &Path) -> Option<String> {
    let advanced_dir = super::advanced_store_dir(repo_root);
    if !advanced_dir.exists() {
        return None;
    }
    super::publication::read_pointer(&advanced_dir).ok().flatten().map(|p| p.generation_id)
}
