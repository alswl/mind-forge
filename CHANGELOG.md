...
- **article new**: default to `blank` template and directory-article output shape. `mf article new "X"` creates `docs/x/01-opening.md` from the minimal blank scaffold.
...
- **article new**: `--file`/`--single-file` opts into the single-file output shape `docs/{slug}.md` for compatibility with legacy workflows.
- **article new**: directory articles are split on `## ` headings — `01-opening.md` + one block file per H2 section.
