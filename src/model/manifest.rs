...
pub struct AdvancedSourceConfig {
    ...
    /// Provider API key reference.  Holds either a literal key or, preferably, a
    /// `${name}` reference resolved against the gitignored `minds-secrets.yaml`
    /// key/value store.  A `${name}` reference is non-sensitive and safe to
    /// commit; it round-trips through `minds.yaml` untouched.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub embedding_api_key: Option<String>,
    /// The resolved literal key held in memory only.  `skip` guarantees it is
    /// never serialized into `minds.yaml`, so a resolved secret cannot leak.
    #[serde(skip)]
    pub embedding_api_key_resolved: Option<String>,
...
impl Default for AdvancedSourceConfig {
    fn default() -> Self {
        Self {
            ...
            embedding_api_key_resolved: None,
