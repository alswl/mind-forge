...
pub fn render_text(block: &ShowBlock) -> String {
    ...
    for field in &block.fields {
        write_field(&mut out, field, max_label_w, 0);
    }
    ...
    for section in &block.sections {
        ...
        for field in &section.fields {
            write_field(&mut out, field, max_label_w, 2);
        }
    }
...
fn write_field(out: &mut String, field: &ShowField, max_label_w: usize, indent: usize) {
...
mod tests {
    ...
    fn key_value_alignment() {
        ...
        let result = render_text(&block);
        let lines: Vec<&str> = result.lines().collect();
        // Both keys should have the same column offset for values
        ...
        // value offsets relative to start of line
